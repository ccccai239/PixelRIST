...
from MGMSA_v1 import MgmsaForCausalLM
...
def parse_args(args):
    ...
    parser.add_argument(
        "--dataset", default="sem_seg||refer_seg||vqa||reason_seg||caption||vg", type=str
    )
    parser.add_argument("--sample_rates", default="9,6,3,1,3,6", type=str)
    parser.add_argument(
        "--sem_seg_data",
...
def main(args):
    ...
    num_added_tokens = tokenizer.add_tokens(["[SEG]",OBJECT_NAME_START])
    args.seg_token_idx = tokenizer("[SEG]", add_special_tokens=False).input_ids[1]#input_ids[1] 
    print('seg_token_idx:',args.seg_token_idx)
    args.obj_token_idx = tokenizer(OBJECT_NAME_START, add_special_tokens=False).input_ids[1] #input_ids[1]
    print('obj_token_idx:',args.obj_token_idx)
    #args.rej_token_idx = tokenizer("[REJ]", add_special_tokens=False).input_ids[1]
    ...
    for n, p in model.named_parameters():
        if any(
            [
                x in n
                for x in ["lm_head", "embed_tokens", "mask_decoder", "text_hidden_fcs","layer_weights"]
            ]
        ):
    ...
    best_score_true, cur_ciou_true = 0.0, 0.0

    if args.eval_only:
        giou, ciou,giou_true,ciou_true = validate(val_loader, model_engine, 0, writer, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.no_eval == False:
            giou, ciou,giou_true,ciou_true = validate(val_loader, model_engine, epoch, writer, args)
...
#            layer_weights=output_dict['layer_weights']
            #print(output_dict['layer_weights'])
