...
def top_k_top_p_filtering(logits, top_k=0, top_p=0.0):
    ...
    return logits_processor(logits)
def parse_args(args):
    ...
    parser.add_argument("--dice_loss_weight", default=0.75, type=float)
    #parser.add_argument("--bce_loss_weight", default=2, type=float)
    parser.add_argument("--focal_loss_weight", default=2.0, type=float)
    parser.add_argument("--boundary_loss_weight", default=0.25, type=float)
...
def main(args):
    ...
    best_score_true, cur_ciou_true = 0.00, 0.00
...
def train(train_loader, model, epoch, scheduler, writer, train_iter, args):
    ...
    mask_focal_losses = AverageMeter("MaskFocalLoss", ":.4f")
    ...
    for global_step in range(args.steps_per_epoch):
        for i in range(args.grad_accumulation_steps):
            ...
            mask_focal_loss = output_dict["mask_focal_loss"]
            ...
            if mask_focal_loss == 0.0:
                mask_focal_losses.update(0.0, input_dict["images"].size(0))
            else:
                mask_focal_losses.update(mask_focal_loss.item(), input_dict["images"].size(0))

            if mask_boundary_loss == 0.0:
                mask_boundary_losses.update(0.0, input_dict["images"].size(0))
            else:
        ...
        if global_step % args.print_freq == 0:
            ...
            if args.local_rank == 0:
                ...
                # )
                writer.add_scalar(
                    "train/mask_focal_loss", mask_focal_losses.avg, global_step
                )
                writer.add_scalar(
                    "train/mask_boundary_loss", mask_boundary_losses.avg, global_step
                )
                writer.add_scalar(
...
def validate(val_loader, model_engine, epoch, writer, args, tokenizer):
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        # For text evaluation
        output_texts = output_dict['logits']

        generated_texts = []

        #遍历每个输出序列
        for output_i in output_texts:
            token_ids = output_i[0].argmax(dim=-1).tolist()
        ...
        target_texts = input_dict["conversation_list"]
        # 遍历生成的文本和目标文本，计算评估指标
        for generated, target in zip(generated_texts, target_texts):
            # Evaluate using BLEU score
            reference = [t.strip().split() for t in target]
            candidate = generated.strip().split()
            bleu_score = sentence_bleu(reference, candidate)
            bleu_scores.append(bleu_score)

            # Evaluate using ROUGE score
            rouge_score = rouge.get_scores(generated, target, avg=True)
            rouge_scores.append(rouge_score)

            # Evaluate using METEOR score
            #meteor_score = meteor_score(generated, target)
    ...
    if args.local_rank == 0:
        ...
        writer.add_scalar("val/bleu", avg_bleu, epoch)
        writer.add_scalar("val/rouge-1", avg_rouge['rouge-1'], epoch)
        writer.add_scalar("val/rouge-2", avg_rouge['rouge-2'], epoch)
        writer.add_scalar("val/rouge-l", avg_rouge['rouge-l'], epoch)
        print("giou: {:.4f}, ciou: {:.4f}".format(giou, ciou))
        print("giou_true: {:.4f}, ciou_true: {:.4f}".format(giou_true, ciou_true))

    #with open(os.path.join(args.log_dir, "generated.json"), "w") as f:
