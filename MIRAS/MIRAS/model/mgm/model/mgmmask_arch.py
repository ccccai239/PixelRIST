...
class MGMMetaModel:
    ...
    def __init__(self, config):
        ...
        self.separate_mm_projector = config.separate_mm_projector #getattr(config, 'separate_mm_projector', True)
    ...
    def initialize_uni_modules(self, model_args, for_eval=False):  
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            if hasattr(model_args, 'model_name_or_path'):
                ...
                model_save_path = model_args.name_or_path #model_path
            ...
            if IS_NEW_TRANSFORMERS:
                ...
                weight_file = json.load(open(os.path.join(model_idx_path, 'pytorch_model.bin.index.jsonpip'), 'r'))['weight_map']
...
class MGMMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, images_aux=None,clip_resize_list=None,is_video=False):
        # #处理注意力掩码
        # vit_attention_mask_llm = None
        ...
        #     vit_attention_mask_llm = F.interpolate(vit_attention_mask[:, None], size=(16, 16), mode="nearest")[:, 0]
        #     vit_attention_mask_llm = vit_attention_mask_llm.flatten(1)
        ...
        if image_grid > 1:
            #images = images.reshape(-1,3,336,336)
            batch_size = images.shape[0]
            #n = images.shape[0] // 3
            #image_list=torch.chunk(images,chunks=n,dim=0)
            #images_ =[image for image in image_list]
            if image_global:
                #images[bs,3,336,336]
                global_images = images[:, -1:].flatten(0,1).contiguous() #取出全局特征 ，维度：[batch_size, 3, 336，336] -> [batch_size, 336，336]
                grid_images = images[:, :-1].flatten(0,1).contiguous() #取出局部特征，维度：[batch_size, 3, 336，336] -> [batch_size*2, 336，336]

                images= torch.cat([grid_images, global_images], dim=0)#维度：[batch_size, 3, 336，336] -> [batch_size*3, 336，336]
                
            else:
                images = images.flatten(0,1).contiguous() #维度转换过程：[batch_size, 3, 336，336] -> [batch_size*3, 336，336]
        
        image_features = self.get_model().get_vision_tower()(images) #（bs*3,576,1024) 
        
        if image_global:
            #print(len(global_images), len(grid_images), len(image_features)) 
            image_feat_global = image_features[-len(global_images):] #取出全局特征 (bs,576,1024)
            image_features = image_features[:len(grid_images)] #取出局部特征 (bs*2,576,1024)


        if isinstance(image_features, list):
            image_features = torch.stack(image_features)

        #patch_num_= int(image_features.shape[1]**0.5) #24
        ...
        if images_aux is not None:
            ...
            if image_global:
                image_aux_features_global = F.interpolate(image_aux_features_raw.float(), 
                                                            ...
                                                            mode='bilinear', 
                                                            align_corners=False).to(dtype=image_aux_features_raw.dtype) #（bs，2880，24，24）
                #进一步修改image_aux_features_global的维度
                # image_aux_features_global = F.interpolate(image_aux_features_raw.float(), 
                #                                              size=(patch_num_, patch_num_), 
                #                                              mode='bilinear', 
                #                                              align_corners=False).to(dtype=image_aux_features_raw.dtype) #(bs,2880,24,24)
                image_feat_global, image_aux_feat_global = self.unified_resampler(image_feat_global, image_aux_features_global)
               #(6,576,1024) (6,576,1024)
            if image_grid > 1:
                #进一步修改image_aux_features_raw的维度 (bs,2880,84,84) -> (bs,2880,24,24)
                # image_aux_features_raw = F.interpolate(image_aux_features_raw.float(), 
                #                             size=(, patch_num_), 
                #                             mode='bilinear', 
                #                             align_corners=False).to(dtype=image_aux_features_raw.dtype) #(bs,2880,24,24)
                image_aux_features_raw = image_aux_features_raw.reshape(*image_aux_features_raw.shape[:2],
                                                                        ...
                                                                        image_grid,
                                                                        image_aux_features_raw.shape[-1]//image_grid) #(bs,2880,2,24,2,24)
                
                image_aux_features_raw = image_aux_features_raw.permute(0, 2, 4, 1, 3, 5).flatten(1,2).flatten(0,1).contiguous()  #(bs*4, 2880, 24, 24)
            image_features, image_aux_features = self.unified_resampler(image_features, image_aux_features_raw)
            #(24,576,1024) (24,576,1024)
            if image_grid > 1:
                total_elements = image_features.numel()
                # 计算新的形状的维度
                new_dim2 = total_elements // (batch_size * image_grid**2)
                image_features = image_features.reshape(batch_size, image_grid**2, 576, new_dim2//576) #(6,4,576,1024)

                #image_features = image_features.reshape(batch_size, image_grid**2, *image_features.shape[1:])
                image_features = image_features.flatten(1,2).contiguous() #(6,2304,1024)
                image_aux_features = image_aux_features.reshape(batch_size, image_grid**2, 576,new_dim2//576)# (6,4,576,1024)
                image_aux_features = image_aux_features.flatten(1,2).contiguous() #(6,2304,1024)
            
            # add global features, [global, local]
            if image_global:
                image_features = torch.cat([image_feat_global, image_features], dim=1) #（bs*3,576,1024) -->(6,2880,1024)
                image_aux_features = torch.cat([image_aux_feat_global, image_aux_features], dim=1) #(6,2880,1024)
             
            # token generation
            image_features = image_features + image_aux_features
            #(bs*3,576,1024)--->(6,2880,1024)
        # process image features after token generation
        if self.separate_mm_projector:
            image_features = self.get_model().out_mm_projector(image_features)
        else:
            image_features = self.get_model().mm_projector(image_features)
        #image_features = self.get_model().mm_projector(image_features) #（bs,2880,4096)
        #(bs*3,576,4096)
    ...
    def unified_resampler(self, images, images_aux):
        ...
        if isinstance(images_aux, list):
            images_aux = torch.stack(images_aux)
        # patchwise with square images
        patch_num = int(images.shape[1]**0.5) #24
        if images_aux.shape[-1] < patch_num :
            patch_size = 1
        else:
            patch_size = images_aux.shape[-1]//patch_num
        # within patch attention
        images_aux = images_aux.permute(0,2,3,1) #（24，2880，12,12） -》（bs*4，12,12，2880）
        images_aux = images_aux.reshape(len(images_aux), patch_num, patch_size, patch_num, patch_size, images_aux.shape[-1]) #（6,24,1,24,1,2880)
        images_aux = images_aux.permute(0,1,3,2,4,5) #（6,24,24,1,1,2880）
        images_aux = images_aux.reshape(len(images_aux), patch_num**2, patch_size**2, images_aux.shape[-1]).contiguous() # (6,576,1,1880)

        # token attention
        if images.shape[0] == images_aux.shape[0]:
            embed_query = self.get_model().vlm_uni_query_projector(images)  #(12,576,1024)
            ...
            images = torch.repeat_interleave(images, 2, dim=0)  # (24,576,1024)
            embed_query = self.get_model().vlm_uni_query_projector(images)  #(12,576,1024)
            #embed_query = torch.repeat_interleave(embed_query, 2, dim=0)  # (24,576,1024)
            embed_aux = self.get_model().vlm_uni_aux_projector(images_aux) #(24,576,1,1024)
            embed_value = self.get_model().vlm_uni_val_projector(images_aux) #(24,576,1,1024)
            #print(embed_query[:,:,None].shape, embed_aux.transpose(-1,-2).shape, embed_value.shape)
            embed_att = embed_query[:,:,None] @ (embed_aux.transpose(-1,-2) / (embed_aux.shape[-1]**0.5)) 
        embed_att = embed_att.nan_to_num()
        embed_feat = (embed_att.softmax(-1) @ embed_value).mean(2) 
        #print(images.shape,embed_feat.shape)
        
        return images, embed_feat

    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, past_key_values, labels, images=None, images_aux=None,clip_resize_list=None
    ):        
        ...
        image_features = self.encode_images(images, images_aux,clip_resize_list) #（bs,576,4096) --- (bs,2880,4096)
        ...
        input_ids = [cur_input_ids[cur_attention_mask] for cur_input_ids, cur_attention_mask in zip(input_ids, attention_mask)] #去掉填充部分
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    # Extend attention_mask to cover the new embeddings
                    #attention_mask[batch_idx] = torch.cat([cur_attention_mask, attention_mask[batch_idx][image_token_indices[i]+1:]], dim=0)
        ...
        return image_features,None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels
