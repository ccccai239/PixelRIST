...
class ALLaVADataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        IMAGE_DIR = os.path.join(image_dir, "vision-Flan/images_191task_1k")
        self.allava_image_root = IMAGE_DIR    #os.path.join(base_image_dir, "coco/train2017")
        with open(os.path.join(image_dir,allava_data)) as f:
            allava_data = json.load(f)
        self.allava_data = allava_data
        self.data_args = args

        print("allava_data: ", len(self.allava_data))
    ...
    def __getitem__(self, idx):
        idx= random.randint(0,len(self.allava_data)-1)
        item = self.allava_data[idx]
        image_path = os.path.join(self.allava_image_root, item["image"].split("/")[-1])
