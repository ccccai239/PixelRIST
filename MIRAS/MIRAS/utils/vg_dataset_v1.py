...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = self.vg_data[ds]['image_path'] #os.path.join(self.VG_IMG_DIR,self.vg_data['images'][ds]['file_name'])
        ...
        #    mask = cv2.imread(masks[i], cv2.IMREAD_GRAYSCALE).astype(np.float32)
        #    mask[mask == 255] = 1  # 将值为255的部分转换为1
